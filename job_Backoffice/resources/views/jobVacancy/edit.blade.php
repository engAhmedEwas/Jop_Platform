        ...
        <form action=" {{route('job-vacancies.update', ['job_vacancy' => $jobVacancy->id, 'redirectToList' => request()->query('redirectToList') ])}} " method="post">
            ...
            <div class="max-w-2xl p-6 mx-auto bg-white rounded-lg shadow-md">
                ...
                <div class="p-6 mb-4 border border-gray-100 rounded-lg shadow-sm bg-gray-50">
                    ...
                    <span>Edit The Job Vacancy Details Below</span>

                    <div class="mt-3 mb-2">
                        <label for="title" class="block text-sm font-medium text-gray-700">Job Title</label>
                        <input type="text" name="title" id="title"
                            class="w-full my-2 border-gray-400 rounded-xl {{ $errors->has('title') ? 'outline-red-500 outline outline-1' : '' }} " value="{{old('title', $jobVacancy->title)}}">
                    ...
                    <div class="mt-3 mb-2">
                        ...
                        <input type="number" name="salary" id="salary"
                            class="w-full my-2 border-gray-400 rounded-xl {{ $errors->has('salary') ? 'outline-red-500 outline outline-1' : '' }} " value="{{old('salary', $jobVacancy->salary)}}">
                    ...
                    <div class="mt-3 mb-2">
                        ...
                        <input type="text" name="location" id="salary"
                            class="w-full my-2 border-gray-400 rounded-xl {{ $errors->has('location') ? 'outline-red-500 outline outline-1' : '' }} " value="{{old('location', $jobVacancy->location)}}">
                    ...
                    <div class="mt-3 mb-2">
                        ...
                        <select name="jobCategory_id" id="jobCategory_id" class="w-full my-2 border-gray-400 rounded-xl {{ $errors->has('jobCategory_id') ? 'outline-red-500 outline outline-1' : '' }} " value="{{old('jobCategory_id')}}">
                            @forelse($categories as $category)
                                <option value="{{ $category->id }}" {{ old('jobCategory_id', $jobVacancy->jobCategory_id) == $company->id ? "selected" : ""}}> {{ $category->name }} </option>
                            @empty
                            @endforelse
                        </select>
                    ...
                    <div class="mt-3 mb-2">
                        ...
                        <textarea rows="4" name="description" id="description" class="w-full my-2 border-gray-400 rounded-xl {{ $errors->has('description') ? 'outline-red-500 outline outline-1' : '' }} " >{{old('description', $jobVacancy->description)}}</textarea>
                ...
                <div class="flex justify-end space-x-2">
                    <button type="submit" class="px-2 py-4 text-white bg-blue-500 border border-blue-500 hover:bg-blue-700 rounded-xl">Update Job Vacancy</button>
